...
if (!isset($_SESSION['user_id']) || ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'supervisor_atc')) {
...
switch ($method) {
    case 'POST':
        ...
        if (isset($data['action']) && $data['action'] === 'generate_challenge') {
            ...
            $sql_participants = "SELECT agente1_id as id FROM versus_entries_atc WHERE date = ? UNION SELECT agente2_id as id FROM versus_entries_atc WHERE date = ?";
            ...
            $sql_counts = "
                ...
                FROM (
                    SELECT agente1_id AS agente_id FROM versus_entries_atc WHERE MONTH(date) = ? AND YEAR(date) = ?
                    UNION ALL
                    SELECT agente2_id AS agente_id FROM versus_entries_atc WHERE MONTH(date) = ? AND YEAR(date) = ?
                ) as matches
                GROUP BY agente_id
            ";
            ...
            $stmt = $conn->prepare("SELECT id, name, photoUrl FROM agentes_atc WHERE id IN (?, ?)");
            ...
            $stmt = $conn->prepare("INSERT INTO versus_entries_atc (date, agente1_id, agente2_id, challenge_description) VALUES (?, ?, ?, 'Desafío del día')");
            ...
            $stmt = $conn->prepare("SELECT * FROM versus_entries_atc WHERE id = ?");
            ...
            $sql_kpi = "SELECT * FROM kpi_entries_atc WHERE date = ? AND agenteId IN (?, ?)";
            ...
            $kpiAgente1 = $kpis[$agente1_id] ?? ['soportesAtendidos' => 0, 'ticketsGenerados' => 0, 'pagosRegistrados' => 0, 'actividadesAsignadas' => 0];
            $kpiAgente2 = $kpis[$agente2_id] ?? ['soportesAtendidos' => 0, 'ticketsGenerados' => 0, 'pagosRegistrados' => 0, 'actividadesAsignadas' => 0];

            $score1 = ($kpiAgente1['soportesAtendidos'] * 20) + ($kpiAgente1['ticketsGenerados'] * 5) + ($kpiAgente1['actividadesAsignadas'] * 0.5) + ($kpiAgente1['pagosRegistrados'] * 1.5);
            $score2 = ($kpiAgente2['soportesAtendidos'] * 20) + ($kpiAgente2['ticketsGenerados'] * 5) + ($kpiAgente2['actividadesAsignadas'] * 0.5) + ($kpiAgente2['pagosRegistrados'] * 1.5);
            ...
            $stmt_update = $conn->prepare("UPDATE versus_entries_atc SET winner_id = ?, loser_id = ?, is_draw = ?, points_change = ? WHERE id = ?");
    ...
    case 'GET':
        if (isset($_GET['date'])) {
            ...
            $sql = "SELECT v.*, a1.name as name1, a1.photoUrl as photo1, a2.name as name2, a2.photoUrl as photo2 
                    FROM versus_entries_atc v 
                    JOIN agentes_atc a1 ON v.agente1_id = a1.id 
                    JOIN agentes_atc a2 ON v.agente2_id = a2.id 
                    WHERE v.date = ?";
    ...
    case 'DELETE':
        ...
        $stmt_delete = $conn->prepare("DELETE FROM versus_entries_atc WHERE id = ?");
