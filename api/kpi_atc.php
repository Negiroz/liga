...
switch ($method) {
    case 'GET':
        ...
        $select_cols = $aggregate ?
            "k.agenteId, k.date, a.name as agenteName,
             SUM(k.soportesAtendidos) as soportesAtendidos,
             SUM(k.ticketsGenerados) as ticketsGenerados,
             SUM(k.pagosRegistrados) as pagosRegistrados,
        ...
        if (isset($_GET['date'])) {
            $date = $_GET['date'];
            $sql = "SELECT $select_cols FROM kpi_entries_atc k JOIN agentes_atc a ON k.agenteId = a.id WHERE k.date = ?" . $group_by;
            ...
            $sql = "SELECT $select_cols FROM kpi_entries_atc k JOIN agentes_atc a ON k.agenteId = a.id WHERE k.date >= ? AND k.date <= ?" . $group_by;
            $stmt = $conn->prepare($sql);
    ...
    case 'POST':
        ...
        try {
            $stmt = $conn->prepare(
                "INSERT INTO kpi_entries_atc (agenteId, date, soportesAtendidos, ticketsGenerados, pagosRegistrados, actividadesAsignadas, actividadesCompletadas) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)
                 ON DUPLICATE KEY UPDATE 
                    soportesAtendidos = VALUES(soportesAtendidos),
                    ticketsGenerados = VALUES(ticketsGenerados),
                    pagosRegistrados = VALUES(pagosRegistrados),
            ...
            foreach ($entries as $entry) {
                 if ($entry['soportesAtendidos'] > 0 || $entry['ticketsGenerados'] > 0 || $entry['pagosRegistrados'] > 0 || $entry['actividadesAsignadas'] > 0 || $entry['actividadesCompletadas'] > 0) {
                     $stmt->bind_param("isiiiii", 
                        $entry['agenteId'], $entry['date'], $entry['soportesAtendidos'], $entry['ticketsGenerados'],
                        $entry['pagosRegistrados'], $entry['actividadesAsignadas'], $entry['actividadesCompletadas']
                    );
                    $stmt->execute();
                    $entries_processed++;
                 }
    ...
    case 'PUT':
        ...
        $conn->begin_transaction();
        try {
            // Modificar para usar UPSERT basado en agenteId y date
            $stmt = $conn->prepare(
                "INSERT INTO kpi_entries_atc (agenteId, date, soportesAtendidos, ticketsGenerados, pagosRegistrados, actividadesAsignadas, actividadesCompletadas) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)
                 ON DUPLICATE KEY UPDATE 
            ...
            foreach ($updates as $update) {
                ...
                $stmt->bind_param("isiiiii",
                    $update['agenteId'],
                    $update['date'],
                    $update['soportesAtendidos'],
                    $update['ticketsGenerados'],
                    $update['pagosRegistrados'],
